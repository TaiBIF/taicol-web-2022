...
const LatestNewsList: React.VFC = () => {
  const [selectedCategory, setSelectCategory] = React.useState<number | string>('all')
  const [page, setPage] = React.useState<number>(1)
  const [total, setTotal] = React.useState<number>(0)
  const pageSize = 3

  const GET_LATEST_NEWS_LIST_URL = `/api/news/latest?cid=${selectedCategory}&page=${page}`;
  const GET_CATEGORY_LIST_URL = `/api/category?type=news`;
  const { data:newsList } = useSWR<NewsListProps>(GET_LATEST_NEWS_LIST_URL);
