import { createFaqFormSchema,updateFaqFormSchema } from 'src/form/faq/saveFaqFormSchema';
...
export default async (req: NextApiRequest, res: NextApiResponse<ResponseData>) => {
  ...
  const mode = req.body.id  ? 'update' : 'create';
	const result = mode == 'update' ? updateFaqFormSchema : createFaqFormSchema
    .safeParse(req.body);
    ...
    if (mode == 'create') {
      ...
      const faq = await Faq.create(insertData);
      ...
      const result = await Faq.update(insertData, { where: { id: req.body.id } });
