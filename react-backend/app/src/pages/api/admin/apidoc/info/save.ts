import { saveApidocInfoFormSchema } from 'src/form/apidoc/saveApidocFormSchema';
...
export default async (req: NextApiRequest, res: NextApiResponse<ResponseData>) => {
  ...
  if (req.method != "POST") res.status(403);
	...
	const result = saveApidocInfoFormSchema.safeParse(req.body);
  ...
  ApidocInfo.truncate()
    ...
    const apidocResponse = await ApidocInfo.create(insertData);
