    ...
    if path := row.path:
        ...
        if path:
            ...
            with conn.cursor() as cursor:
                ...
                for r in results:
                    df.loc[i, f'{rank_map[r[0]].lower()}'] = r[1]
                    df.loc[i, f'{rank_map[r[0]].lower()}_c'] = r[2]
...
df.to_csv(f"物種名錄_物種_{today.strftime('%Y%m%d')}.csv")
...
query = """SELECT tn.id, n.name, tn.rank_id, tn.name, an.name_author, an.formatted_name,
...
names.to_csv(f"物種名錄_學名_{today.strftime('%Y%m%d')}.csv")
