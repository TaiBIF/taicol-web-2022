...
const taxonCountList:TaxonCountProps[] = [
  {
    ...
    tooltip: <Translation>{t => <h1>{t('包含種下分類群')}</h1>}</Translation>,
  },
  {
    img: '/static/image/ssicon02.svg',
    title: '收錄學名數',
    enTitle:<Translation>{t => <h1>{t('SCIENTIFIC NAMES')}</h1>}</Translation>,
    count: 0,
    type:'name',
    CircleClassName: 'cir-line2',
    tooltip: <Translation>{t => <h1>{t('包含種上階層學名')}</h1>}</Translation>,
  },
  {
    img: '/static/image/ssicon03.svg',
    title: '收錄文獻數',
    enTitle:<Translation>{t => <h1>{t('REFERENCES')}</h1>}</Translation>,
    count: 0,
    type:'reference',
    CircleClassName: 'cir-line1',
  }
